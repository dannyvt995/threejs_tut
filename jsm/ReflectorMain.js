...
class ReflectorMain extends Mesh {
	...
	constructor( geometry, options = {} ) {
		...
		const shader = options.shader || ReflectorMain.ReflectorShader;
		...
		material.uniforms[ 'tHeightMap' ].value = null;
		...
		this.onBeforeRender = function ( renderer, scene, camera ) {
			...
			if ( view.dot( normal ) > 0 ) {
					// phía dưới gương
					this.material.uniforms[ 'opacity' ].value = .5;
					return
			}else{
...
ReflectorMain.ReflectorShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			gl_FragColor = vec4(  blendOverlay( base.rgb, vec3(.5) ) * 1.2 + outgoingLight * .75, opacity);
		}`
};

export { ReflectorMain };