...
export class GenerateMeshBVHWorker extends WorkerBase {
	...
	constructor() {
	    ...
	    let worker;
		try {
			const workerUrl = new URL('./generateMeshBVH.worker.js', import.meta.url);
			console.log('Worker URL:', workerUrl.href);  // Log the full URL
			worker = new Worker(workerUrl, { type: 'module' });
		} catch (error) {
			console.error('Failed to create worker:', error);
			throw new Error(`Unable to create GenerateMeshBVHWorker: ${error.message}`);
		}
	...
	runTask( worker, geometry, options = {} ) {
		...
		return new Promise( ( resolve, reject ) => {
			...
			console.log("runTask",geometry,worker)
			worker.onerror = e => {
